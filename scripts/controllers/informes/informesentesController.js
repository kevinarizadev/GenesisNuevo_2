...
function($scope,consultaHTTP,notification,cfpLoadingBar,$http,$window,$filter) {
  ...
  $scope.generaReporte = function(){
    switch($scope.tiporeporte){
      ...
      case "7":
      var ReportTitle=" Total Radicacion Prestadores", ShowLabel=true;
        var fecha_inicio = $filter('date')(new Date($scope.fecha_inicio), 'dd/MM/yyyy');
        var fecha_final = $filter('date')(new Date($scope.fecha_final), 'dd/MM/yyyy');
      ...
      // contratacion
          case "11":
            window.open('php/Informes/redcontratacion.php?ann=' + $scope.anno + '&per=' + $scope.periodo);
            $scope.consmayudaaut = false;
            $scope.consproductos = false;
            $scope.consmovempresa = false;
            break;
         ...
         case "50":
            window.open('php/Informes/redcontratacion_sinClasificacion.php?ann=' + $scope.anno + '&per=' + $scope.periodo);
            $scope.consmayudaaut = false;
            $scope.consproductos = false;
            $scope.consmovempresa = false;
            break;
      ...
      case "56":
         ...
         break;
           ...
           case "58":
            ...
            $scope.mostrar_radicado_contratos=false;
            if (
              (($scope.fecha_inicio == undefined) || ($scope.fecha_inicio == null) || ($scope.fecha_inicio == "")) ||
              (($scope.fecha_final == undefined) || ($scope.fecha_final == null) || ($scope.fecha_final == "")) ||
              (($scope.regimen == undefined) || ($scope.regimen == null) || ($scope.regimen == "")||  ($scope.regimen == "X")) ||
              (($scope.contrato == undefined) || ($scope.contrato == null) || ($scope.contrato == ""))
            )
     ...
     case "14":
      ...
      case "44":
      var fecha_inicio = $filter('date')(new Date($scope.fecha_inicio), 'dd/MM/yyyy');
      window.open('php/Informes/repautfuncionario.php? &fecha_inicio='+fecha_inicio);
      $scope.consmayudaaut=false;
      $scope.consproductos= false;
      $scope.consmovempresa=false;
      break;
      ...
      case "45":
      window.open('php/Informes/reportepqr.php?anno='+$scope.anno+'&periodo='+$scope.periodo);
      $scope.consmayudaaut=false;
      $scope.consproductos= false;
      $scope.consmovempresa=false;
      break;
       ...
       var fecha_inicio = $filter('date')(new Date($scope.fecha_inicio), 'dd/MM/yyyy');
       var fecha_final = $filter('date')(new Date($scope.fecha_final), 'dd/MM/yyyy');
       window.open('php/Informes/reporte_autorizaciones_circular_017_ST015_por_mes.php? &fecha_inicio=' + fecha_inicio +
      ...
      break;    
       ...
       var fecha_inicio = $filter('date')(new Date($scope.fecha_inicio), 'dd/MM/yyyy');
       var fecha_final = $filter('date')(new Date($scope.fecha_final), 'dd/MM/yyyy');
       window.open('php/Informes/Reporte_Homologacion_medicamentos_autorizaciones_por_mes.php? &fecha_inicio=' + fecha_inicio +
                '&fecha_fin=' + fecha_final);
      $scope.shw_annos = false;
      $scope.shw_periodo = false;
      $scope.consmayudaaut = false;
    ...
    case "53":
              var fecha_inicio = $filter('date')(new Date($scope.fecha_inicio), 'dd/MM/yyyy');
              var fecha_final = $filter('date')(new Date($scope.fecha_final), 'dd/MM/yyyy');
              window.open('php/Informes/Reporte_Presupuestos_Maximos_Medicamentos_por_mes.php? &fecha_inicio=' + fecha_inicio +
                '&fecha_fin=' + fecha_final);
                $scope.shw_annos = false;
                $scope.shw_periodo = false;
                $scope.consmayudaaut = false;
     ...
     case "59":
           window.open('php/Informes/autorizaciones_pos_control_interno.php?anno=' + $scope.anno + '&periodo=' + $scope.periodo);
           //$scope.shw_annos = false;
          // $scope.shw_periodo = false;
      break;
      case "60":
            window.open('php/Informes/autorizaciones_altocosto_control_interno.php?anno=' + $scope.anno + '&periodo=' + $scope.periodo); 
           // $scope.shw_annos = false;
           // $scope.shw_periodo = false;
      break;
      case "61":
            window.open('php/Informes/tipificacionradicacion.php?anno=' + $scope.anno + '&periodo=' + $scope.periodo);
            $scope.consmayudaaut = false;
            $scope.consproductos = false;
            $scope.consmovempresa = false;
      break;
      ...
      case "65":
          window.open(
            "php/Informes/mesaayudadigiturno.php?anno="+$scope.anno.toString()+"&periodo="+$scope.periodo.toString());
    ...
    if ($scope.numero_contrato == '' || $scope.ubicacion == '') {
      ...
      $http({
        ...
        if (response.data.length > 0) {
          ...
          swal('Información', "No se registraron usuarios asignados al escenario y al contrato digitados.", 'info');
          $scope.infousuarios = "";
