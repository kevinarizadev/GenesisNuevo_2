...
$scope.FormatSoloTextoNumero = function (NID) {
  ...
  input.value = valor.replace(/[^\wÑñ,.-\s]/g, '');
    ...
    $scope.validarafiliado = function () {
      if($scope.buscar_tipo_documento == null || $scope.buscar_tipo_documento == "" || $scope.buscar_tipo_documento == undefined ||
      $scope.buscar_numero == null || $scope.buscar_numero == "" || $scope.buscar_numero == undefined) {
        swal({
          title: "Mensaje",
          text: "Tipo de Documento y Numero de Documento Son Obligatorios",
          type: "info",
        })
      } else {
        $http({
          ...
          data: {
            ...
            tipo_doc: $scope.buscar_tipo_documento,
            numero_doc: $scope.buscar_numero
          }
        }).then(function (response) {
          var datos = response.data
            if (datos.CODIGO == 1){
              ...
              $scope.datosRG.tipo_doc = datos.DTIPO,
              $scope.datosRG.numero_doc = datos.DOCUMENTO,
              ...
              swal({
               ...
               $scope.deshabilitarcampo = true;
              $scope.deshabilitarcampo1 = true;
              var fechanac = datos.FNACIMIENTO.split('/')
              var fechaconver = fechanac[2]+'/'+fechanac[1]+'/'+fechanac[0];
              $scope.verformulario = true;
              $scope.verbusqueda = false;
              $scope.datosRG.tipo_doc = datos.TIPODOCUMENTO,
              $scope.datosRG.numero_doc = datos.DOCUMENTO,
              $scope.datosRG.fecha_nacimiento = new Date(fechaconver),
              $scope.datosRG.fechaexp = datos.FEXPEDICION,
              $scope.datosRG.pnombre = datos.NOMBRE1,
              $scope.datosRG.snombre = datos.NOMBRE2,
              $scope.datosRG.papellido = datos.APELLIDO1,
              $scope.datosRG.sapellido = datos.APELLIDO2,
              $scope.datosRG.sexo = datos.SEXO,
              $scope.datosRG.tipo_sangre = datos.TSANGREE,
              $scope.datosRG.tipo_rh = datos.FRH,
              $scope.datosRG.condicion_salud = "",
              $scope.datosRG.departamento = datos.DEPARTAMENTO,
              setTimeout(() => {
                $scope.datosRG.municipio = datos.MUNICIPIO
              }, 500);
              $scope.datosRG.direccion = datos.DIRECCION,
              $scope.datosRG.barrio = datos.BARRIO,
              $scope.datosRG.celular = datos.TELEFONO,
              $scope.datosRG.correo = datos.CORREO,
              $scope.lista_municipio_change(datos.DEPARTAMENTO);
              swal({
                title: "Mensaje",
                text: datos.MENSAJE,
                type: "success",
  ...
  $scope.cancelarregistro = function(){
    ...
    $scope.Registrar_Promocion = function () {
      $scope.Validar_Campos_Nuevo_Funcionario().then(function (result) {
        ...
        if(!result){
       //    var numerocelular = $scope.datosRG.celular
       ...
       // } else {
        ...
        // } else {
              var DatosRG = {
                ...
                codigo_sat :$scope.datosRG.codigo_sat,
     ...
     $scope.Validar_Campos_Nuevo_Funcionario = function () {
      return new Promise((resolve) => {
        if ($scope.datosRG.codigo_sat == undefined || $scope.datosRG.codigo_sat == null || $scope.datosRG.codigo_sat == '') {
          document.querySelector("#codigo_sat1").classList.add("red-text"); resolve(true);
        } 
