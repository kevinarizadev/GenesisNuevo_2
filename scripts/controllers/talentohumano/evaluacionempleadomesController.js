  ...
  .controller('evaluacionempleadomesController', ['$scope', '$http',
    function ($scope, $http) {
      ...
      $scope.Inicio = function () {
        ...
        //////////////
        if ($scope.Rol_Cedula == "1042454684" || $scope.Rol_Cedula == '72228076' || $scope.Rol_Cedula == '1140842461' || $scope.Rol_Cedula == '1192773718' || $scope.Rol_Cedula == '1048219486') {
      ...
      $scope.p_obtener_estado_modulo = function () {
        $http({
          method: 'POST',
          ...
          if (data.toString().substr(0, 3) != '<br') {
            if (data != undefined) {
              $scope.Val_Estado_Modulo = data.ESTADO;
              if (data.ESTADO == 'I') {
                $scope.Form.Status = 3;
                swal(
                  '¡Importante!',
      ...
      $scope.p_actualizar_estado_modulo = function () {
        $http({
          method: 'POST',
          url: "php/talentohumano/evaluacionempleadomes.php",
          data: {
            function: 'p_actualizar_estado_modulo',
            estado: $scope.Val_Estado_Modulo == 'A' ? 'I' : 'A'
          }
        }).then(function ({ data }) {
          if (data.toString().substr(0, 3) != '<br') {
            if (data != undefined) {
              if (data.CODIGO == '0') {
                ...
                swal(
                  '¡Importante!',
                  'Ocurrio un error, informar al area TIC',
                  'info'
                ).catch(swal.noop);
              }
            }
      ...
      $scope.validar_seleccion_empleado = function () {
        $http({
          method: 'POST',
          url: "php/talentohumano/evaluacionempleadomes.php",
          data: {
          ...
          if (response.data.toString().substr(0, 3) != '<br') {
            if (response.data != undefined) {
              if (response.data.Codigo != 0) {
                if (response.data.Codigo == 2) { $scope.Form.Status = 3; }
                swal(
                  '¡Mensaje!',
            ...
            $http({
              ...
              if (response.data.toString().substr(0, 3) != '<br') {
                if (response.data.Codigo != undefined && response.data.Codigo == 0) {
                  ...
                  setTimeout(() => { $scope.validar_seleccion_empleado(); }, 3000)
                } else {
                  swal(
                    '¡Ocurrio un error!',
                    response.data.Nombre,
      ...
      $scope.Descarga_Historial = function () {
        ...
        swal({
          ...
          swal({
            ...
            const date = new Date(2022, parseInt(mes) - 1, 1);  // 2009-11-10
            const month = date.toLocaleString('default', { month: 'long' });
            var MesAno = (mes < 10 ? '0' + mes : mes) + '/' + anio, Mes = month.toString().toUpperCase();
            window.open('views/talentohumano/formatos/evaluacionempleadomes.php?Mes=' + Mes + '&MesAno=' + MesAno, '_blank', "width=1080,height=1100");
          });
