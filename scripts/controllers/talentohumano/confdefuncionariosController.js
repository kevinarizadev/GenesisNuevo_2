  ...
  .controller('confdefuncionariosController', ['$scope', '$http',
    function ($scope, $http) {
            ...
            if (response.data.toString().substr(0, 3) != '<br') {
              if (response.data[0] != undefined) {
                if (response.data.length == 1 && response.data[0].Codigo == undefined) {
                  ...
                  $scope.Form.Correo = response.data[0].Email;
                  $scope.Form.Status = 1;
                } else {
                  $scope.Form.Status = 0;
                  swal({
      ...
      $scope.Cambiar_Rol = function () {
        ...
        var html = '<input list="Roles" name="browser" id="rol_usu" placeholder="Seleccione..."><datalist id="Roles">';

        $scope.Form.Roles.forEach(e => {
        ...
        swal({
          ...
          console.log(cod)
          var rol_usu= document.querySelector('#rol_usu').value;
          console.log(rol_usu)
          if (cod && rol_usu) {
            cod = rol_usu.split('-')[0].trim();
      ...
      $scope.Cambiar_Correo = function () {
        swal({
          title: 'Configurar Correo Institucional',
          input: 'text',
          inputPlaceholder: 'Escribe la nueva contraseña...',
          showCancelButton: true,
          allowOutsideClick: false,
          inputValue: '@cajacopieps.com',
          inputAttributes: {
            id: "textarea_swal_Pass",
            onkeyup: "Format_Pass()"
          }
        }).then(function (result) {
          $scope.Observacion_correo = result;
              swal({ title: 'Cargando...', allowOutsideClick: false }).catch(swal.noop);
              swal.showLoading();
              $http({
                method: 'POST',
      ...
      const data = '{"tipo_identificacion": "CC","identificacion": "1","primer_nombre": "PRUEBA","segundo_nombre": "PRUEBA","primer_apellido": "PRUEBA","segundo_apellido": "PRUEBA","genero": "M","fecha_nacimiento": "","departamento": "","ciudad": "","regimen": "","zona": "","celular": "","telefono": "","correo_electronico": "","direccion": ""}';

      // const response = await fetch('https://healthgroup-test-6798383.dev.odoo.com/api/private/Aurora/agendar_cita_automatica', {
      //    method: 'PUT',
      //    headers: {
      //     'accept': '*/*',
