...
angular.module('GenesisApp')
  .controller('ModalcolmenaController', ['$scope', '$http', 'ngDialog', 'notification', '$timeout', '$q', 'upload', 'communication', '$controller', '$rootScope', '$window',
    function ($scope, $http, ngDialog, notification, $timeout, $q, upload, communication, $controller, $rootScope, $window) {
      ...
      $scope.actualizarFechaNotifica = function (date) {
        if ($scope.fechaActual2() == true) {
      ...
      function getBase64(file) {
        return new Promise(function (resolve, reject) {
          ...
          reader.onerror = function () { reject("0"); };
          reader.readAsDataURL(file[0]);
        });
      }
      function respuestaUrl(element, location = "Reportes") {
        return new Promise(function (resolve, reject) {
          $http({
            method: 'POST',
            url: "php/planeacion/colmena.php",
      ...
      $scope.guardarReporte = function (fecha) {
        if ($scope.reporte.codigo_reporte != "" && $scope.reporte.objetivo != "" && $scope.reporte.tipo_archivo != "" && $scope.reporte.entes != "" && $scope.reporte.tipo_norma != "" && $scope.reporte.revisa != "" && $scope.reporte.elabora != "" && $scope.reporte.reporta != "" && fecha != "" && $scope.reporte.hora != "" && $scope.reporte.frecuencia != "") {
          ...
          if ($scope.filesArray[0].file.length > 0) {
            ...
            $scope.norma = getBase64($scope.filesArray[0].file[0].files).then(function (result) {
              $scope.filesArray[0].base64 = result;
              $scope.filesArray[0].name = $scope.url_auto($scope.filesArray[0].name);
              return respuestaUrl($scope.filesArray[0], "Normas");
            }).catch(reason => {
              swal('Mensaje', 'No se pudo obtener el base64 del archivo: ' + reason, 'warning');
              $scope.norma = Promise.resolve("");
          ...
          if ($scope.filesArray[1].file.length > 0) {
            ...
            $scope.anexo = getBase64($scope.filesArray[1].file[0].files).then(function (result) {
              $scope.filesArray[1].base64 = result;
              $scope.filesArray[1].name = $scope.url_auto($scope.filesArray[1].name);
              return respuestaUrl($scope.filesArray[1], "Anexos");
            }).catch(reason => {
              swal('Mensaje', 'No se pudo obtener el base64 del archivo: ' + reason, 'warning');
              $scope.anexo = Promise.resolve("");
          ...
          if ($scope.norma != undefined && $scope.anexo != undefined) {
            Promise.all([$scope.norma, $scope.anexo]).then(values => {
              console.log(values);
              $scope.filesArray[0].url = values[0];
              $scope.filesArray[1].url = values[1];
              $scope.reporte = {
                numero: "0",
                codigo_reporte: $scope.reporte.codigo_reporte,
              ...
              console.log(angular.toJson($scope.reporte));
              $http({
                method: 'POST',
                url: "php/planeacion/colmena.php",
                data: {
              ...
              swal.close();
              swal('Mensaje', 'No se pudo subir los archivos al FTP file:' + reason, 'warning');
            });
          } else {
            swal.close();
            swal('Mensaje', 'No se pudo subir los archivos al FTP variables undefined', 'warning');
          }
        } else {
          swal('Campos vacios', 'Por favor, llenar todos los campos obligatorios *', 'warning');
        }
      ...
      $scope.editarReporte = function (fecha) {
        ...
        if ($scope.reporte.codigo_reporte != "" && $scope.reporte.objetivo != "" && $scope.reporte.tipo_archivo != "" && $scope.reporte.entes != "" && $scope.reporte.tipo_norma != "" && $scope.reporte.elabora != "" && $scope.reporte.revisa != "" && $scope.reporte.reporta != "" && fecha != "" && $scope.reporte.hora != "" && $scope.reporte.frecuencia != "") {
          swal({
            html: '<div class="loading"><div class="default-background"></div><div class="default-background"></div><div class="default-background"></div></div><p style="font-weight: bold;">Cargando.</p>',
            width: 200,
            allowOutsideClick: false,
          ...
          if ($scope.norma != undefined && $scope.anexo != undefined) {
            Promise.all([$scope.norma, $scope.anexo]).then(values => {
              ...
              $http({
                ...
                data: {
                  function: 'insertarReporte',
                  json: angular.toJson($scope.reporte)
                }
              }).then(function (response) {
                if (response.data.Codigo == 0) {
                  swal.close();
      ...
      $scope.cargarReporte = function (fecha_cargue, numero_reporte) {
        if (fecha_cargue != "" && $scope.reporte.link_cargue != "" && $scope.reporte.link_cargue != null && $scope.filesArray[2].file[0] != undefined) {
          ...
          getBase64($scope.filesArray[2].file[0].files).then(function (result) {
            ...
            respuestaUrl($scope.filesArray[2], "Reportes").then(function (result_url) {
              $scope.filesArray[2].url = result_url;
              $scope.reporte = {
              ...
              swal('Mensaje', 'No se pudo subir el archivo al FTP code:' + reason, 'warning');
            });
          }).catch(reason => {
      ...
      $scope.loadFile = function (value) {
        ...
        $scope.filesArray[value] = { file: $scope.panel.file, name: extencion[0], ext: $scope.panel.ext, id: value, base64: "" };
      }
