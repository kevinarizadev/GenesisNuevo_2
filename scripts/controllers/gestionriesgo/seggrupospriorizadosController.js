...
angular.module('GenesisApp')
  .controller('seggrupospriorizadosController', ['$scope', '$http', '$filter', '$q', 'afiliacionHttp', 'ngDialog',
    function ($scope, $http, $filter, $q, afiliacionHttp, ngDialog) {
      console.clear();
      ...
      $scope.Cargar_Formulario = function (X) {
        ...
        setTimeout(function () {
          $http({
            ...
            data: { function: 'actualizable', tipo: X.TIPO_DOC, documento: X.NUM_DOC }
          }).then(function (response) {
            $scope.respues_actualizar = response.data.mensaje;
            if ($scope.respues_actualizar == "S") {
              sessionStorage.setItem("tipo", X.TIPO_DOC);
              sessionStorage.setItem("doc", X.NUM_DOC);
              ngDialog.open({
      ...
      $scope.Set_Educacion = function () {
        ...
        if ($scope.Form.Cod_Cohorte == 'VC') {
          $scope.Form.Op_Educaciones = "Recibió educación sobre derechos y deberes, remisión a protocolo de atención en salud con enfoque diferencial. Educación sobre estilos de vida saludables y remisión a ruta de promoción y mantenimiento de acuerdo a curso de vida.";
        }
      }
      ...
      $scope.Abrir_Modal_Soportes = function () {
        $scope.DescargarArchivo($scope.Form.Ruta, $scope.Form.Ftp);
        (function () {
...
function Con() {
  ...
  valor = valor.replace(/(\r\n|\n|\r)/g, ' ');
  valor = valor.replace(/[\t\n]+/g,' ');
  input.value = valor.trim().toString().toUpperCase();
}
